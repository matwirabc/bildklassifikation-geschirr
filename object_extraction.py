...
def generate_mask_with_hsv_threshold(im, sign=None,hue_threshold=None, saturation_threshold=None, value_threshold=None):
    ...
    # Ensure float in [0, 1] BEFORE rgb_to_hsv (avoids NumPy 2 copy=False error)
    im = np.asarray(im)
    if np.issubdtype(im.dtype, np.integer):
        im_rgb = im.astype(np.float32) / 255.0
    else:
        im_rgb = im.astype(np.float32)
        
    im_hsv = rgb_to_hsv(im_rgb) * np.array([255, 255, 255], dtype=np.float32)[None, None, :] 
    
    mask = np.ones(im_hsv.shape[:2], dtype=bool)
    
    if hue_threshold is not None:
...
def morphology_transform(mask,shape = square,erosion_size=5,dilation_size=50):
    ...
    """
    # Fill holes caused by reflections inside the object
    mask = binary_fill_holes(mask)

    # Use closing to connect parts separated by reflections at the boundaries
    if dilation_size > 0:
        mask = closing(mask, shape(max(1, dilation_size // 4)))
    ...
    # Final fill to ensure solid objects
    mask = binary_fill_holes(mask)
    return mask

def create_mask(im,fraction_of_rows_to_remove=0.0,fraction_of_cols_to_remove=0.1,hue_threshold=None,saturation_threshold=None,value_threshold=100,erosion_size=5,dilation_size=50,value_to_fill=0,**kwargs):
    """
...
def image_preprocessing(im, fraction_of_rows_to_remove=0.0, fraction_of_cols_to_remove=0.1, 
                        hue_threshold=None, saturation_threshold=None, value_threshold=100, 
                        erosion_size=5, dilation_size=50, value_to_fill=0, **kwargs):
    """
    ...
    morphed_mask,im_filled = create_mask(im,
                                         fraction_of_rows_to_remove=fraction_of_rows_to_remove,
                                         fraction_of_cols_to_remove=fraction_of_cols_to_remove,
...
def process_arguments(args):
    ...
    kwargs = {'fraction_of_rows_to_remove':fraction_of_rows_to_remove,'fraction_of_cols_to_remove':fraction_of_cols_to_remove,'hue_threshold':hue_threshold,
            ...
            'outputpath':outputpath_Ausschnitte,'preprocessing_resolution':preprocessing_resolution,'value_to_fill':value_to_fill,'write_summary_file':write_summary_file, 'num_cores':num_cores}
...
if __name__ == "__main__":
    ...
    imgfns, kwargs = process_arguments(args)
    main(imgfns, kwargs)
